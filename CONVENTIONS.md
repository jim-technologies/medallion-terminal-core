...
|---|------------|--------|-------|
| 1 | Deps via flox: `.flox/env/manifest.toml` installs node + pnpm (+ buf) | Conforms | `[install]` pins `nodejs_24` 24.20.0 (in its own package group), `pnpm` 11.27.0 (equal to package.json `packageManager`, asserted by `scripts/check-version.mjs`) and `buf` 1.72.0; the Flox lockfile (`.flox/env/manifest.lock`) resolves them for the three systems the catalog serves them on. `flox activate -- make validate` runs clean. |
...
| 14 | jim-technologies Makefile contract (`MAKEFILE-CONTRACT.md`) | Conforms | Verbs: `help` / `fmt` / `test` (+ `test-unit` / `test-storybook` / `test-browser` sub-verbs) / `validate` / `build` / `generate` / `release`, all self-documented via `## comment`. `validate` is the only gate verb (`check` and `ci:verify` are gone) and adds the public-surface guard (`scripts/public-surface-check`, the shared implementation every public jim-technologies repository runs, with its own self-test `scripts/public-surface-check-test` and per-repository exceptions in `.public-surface-allow`) and the version gate (`scripts/check-version.mjs`: `VERSION` against the `package.json` version and the first `CHANGELOG.md` release heading under `## [Unreleased]`, `packageManager` against the Flox-locked pnpm, a declared `engines.node` floor). `release` (`scripts/release.mjs`) is a confirmed-only maintainer action (`RELEASE_CONFIRM=yes`) that refuses a dirty tree, an unpushed HEAD, a red release-mode version gate (which also requires an empty `[Unreleased]`) or an existing tag, and then only creates and pushes the annotated `vVERSION` tag (Git-only distribution). No `deploy` target, no CI secrets, no encrypted-secret store; all dependencies resolve from the public npm registry. |
