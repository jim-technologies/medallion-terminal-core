  ...
  return (
    <div className="ready-showcase stripe-showcase">
      <aside className="stripe-sidebar" aria-label="Stripe navigation">
        ...
        <div className="stripe-sidebar-footer"><button><OperationalShowcaseIcon name="settings" size={16} />Settings</button></div>
      </aside>
      ...
      <div className="stripe-workspace">
        ...
        <main className="stripe-main">
          {section === 'overview' && (
            <>
              <div className="ready-page-heading stripe-page-heading">
                ...
                <div className="ready-heading-actions"><button className="stripe-period-button">Last 7 days <OperationalShowcaseIcon name="chevron-down" size={13} /></button></div>
              </div>
              <section className="stripe-overview-card">
                <div className="stripe-overview-metrics">
                  <StripeOverviewMetric label="Gross volume" value={formatOperationalCurrency(86_420)} change="+12.8%" values={[12, 18, 15, 28, 24, 34, 42, 40, 53, 61, 72]} />
                  <StripeOverviewMetric label="Net volume" value={formatOperationalCurrency(82_914)} change="+11.9%" values={[10, 17, 16, 22, 26, 31, 38, 37, 49, 57, 67]} />
                ...
                <div className="stripe-volume-chart">
                  ...
                  <div className="stripe-chart-plot">
                    ...
                    <OperationalShowcaseSparkline values={[8, 11, 9, 14, 19, 17, 26, 22, 31, 29, 38, 42, 49]} color="#6557d8" height={190} />
              ...
              <div className="stripe-overview-grid">
                <section className="stripe-card">
                  ...
                  <div className="stripe-balance"><span>Available to pay out</span><strong>{formatOperationalCurrency(48_621, { cents: true })}</strong><small><i />Expected Jul 21</small></div>
                  <div className="stripe-balance-row"><span>Pending</span><strong>{formatOperationalCurrency(12_408, { cents: true })}</strong></div>
                ...
                <section className="stripe-card">
                  ...
                  <div className="stripe-recovery">
                    ...
                    <div><strong>{formatOperationalCurrency(7_840)}</strong><span>recovered revenue</span><small>24 of 31 failed payments recovered</small></div>
                  </div>
                ...
                <section className="stripe-card stripe-attention-card">
                  ...
                  <button onClick={() => setSection('disputes')}><i className="warning"><OperationalShowcaseIcon name="warning" size={15} /></i><span><strong>1 dispute needs a response</strong><small>Evidence due Jul 23</small></span><OperationalShowcaseIcon name="chevron-right" size={14} /></button>
                  <button><i><OperationalShowcaseIcon name="shield" size={15} /></i><span><strong>Verify business representative</strong><small>Complete by Aug 2</small></span><OperationalShowcaseIcon name="chevron-right" size={14} /></button>
                  <button><i><OperationalShowcaseIcon name="bank" size={15} /></i><span><strong>Confirm payout account</strong><small>Bank ending in 0284</small></span><OperationalShowcaseIcon name="chevron-right" size={14} /></button>
                </section>
              </div>
            </>
          )}
          ...
          {section === 'payments' && !selectedPayment && (
            <>
              ...
              <div className="stripe-payment-stats">
                <div><span>Net volume</span><strong>{formatOperationalCurrency(netVolume, { cents: true })}</strong><small>From visible successful payments</small></div>
                <div><span>Acceptance rate</span><strong>96.8%</strong><small><OperationalShowcaseIcon name="arrow-up" size={11} />1.2% vs prior period</small></div>
                <div><span>Refunded</span><strong>{formatOperationalCurrency(payments.filter(payment => payment.status === 'Refunded').reduce((sum, payment) => sum + payment.amount, 0), { cents: true })}</strong><small>{payments.filter(payment => payment.status === 'Refunded').length} payment</small></div>
              </div>
          ...
          {section === 'payments' && selectedPayment && (
            <>
              <div className="stripe-detail-breadcrumb"><button onClick={() => setSelectedPaymentId('')}><OperationalShowcaseIcon name="chevron-left" size={15} />Payments</button></div>
              <div className="stripe-payment-detail-heading">
                <div><span className="stripe-detail-icon"><OperationalShowcaseIcon name="check" size={22} /></span><div><h1>{formatOperationalCurrency(selectedPayment.amount, { cents: true })} USD</h1><span><StripeStatus label={selectedPayment.status} />{selectedPayment.createdAt}</span></div></div>
                <div className="ready-heading-actions"><button className="ready-button secondary"><OperationalShowcaseIcon name="more" size={15} />More</button><button className="ready-button primary">Refund</button></div>
              </div>
                ...
                <div>
                  <section className="stripe-card stripe-detail-summary">
                    ...
                    <div className="stripe-detail-timeline">
                      <div><i className="good"><OperationalShowcaseIcon name="check" size={13} /></i><span><strong>Payment succeeded</strong><small>{selectedPayment.createdAt}</small></span><em>{formatOperationalCurrency(selectedPayment.amount, { cents: true })}</em></div>
                      <div><i><OperationalShowcaseIcon name="shield" size={13} /></i><span><strong>Payment authenticated</strong><small>3D Secure was not required</small></span></div>
                      <div><i><OperationalShowcaseIcon name="money" size={13} /></i><span><strong>Payment started</strong><small>Using {selectedPayment.method}</small></span></div>
                    </div>
                  </section>
                  <section className="stripe-card stripe-detail-summary">
          ...
          {section === 'billing' && (
            <>
              ...
              <div className="stripe-billing-stats">
                <div><span>Monthly recurring revenue</span><strong>{formatOperationalCurrency(monthlyRecurring)}</strong><small><OperationalShowcaseIcon name="arrow-up" size={11} />8.4% this month</small></div>
              ...
              <section className="stripe-table-card">
                ...
                <div className="ready-panel-toolbar stripe-table-toolbar"><label className="ready-search-field"><OperationalShowcaseIcon name="search" size={15} /><input placeholder="Search subscriptions" /></label><button><OperationalShowcaseIcon name="filter" size={14} />Filter</button></div>
                <div className="stripe-subscription-table">
                  ...
                  {subscriptions.map(subscription => (
                    <button key={subscription.id}>
                      ...
                      <span>{formatOperationalCurrency(subscription.amount, { cents: true })} / {subscription.interval}</span>
          ...
          {section === 'disputes' && (
            <>
              <div className="ready-page-heading stripe-page-heading">
                ...
                <button className="ready-button secondary"><OperationalShowcaseIcon name="download" size={14} />Export</button>
              </div>
              <div className="stripe-dispute-banner"><OperationalShowcaseIcon name="warning" size={20} /><div><strong>1 dispute needs a response</strong><span>Submit evidence by July 23 to avoid an automatic loss.</span></div><button>Review now</button></div>
              <div className="stripe-dispute-stats">
                <div><span>Disputed volume</span><strong>{formatOperationalCurrency(disputes.reduce((sum, dispute) => sum + dispute.amount, 0))}</strong><small>Last 12 months</small></div>
                <div><span>Dispute rate</span><strong>{formatOperationalPercent(0.0038, 2)}</strong><small>Below 0.75% monitoring level</small></div>
                <div><span>Win rate</span><strong>68.4%</strong><small><OperationalShowcaseIcon name="arrow-up" size={11} />4.2% year over year</small></div>
              </div>
              <section className="stripe-table-card">
                ...
                <div className="stripe-dispute-table">
                  ...
                  {disputes.map(dispute => (
                    <button key={dispute.id}>
                      <strong>{formatOperationalCurrency(dispute.amount, { cents: true })} USD</strong>
...
function StripeOverviewMetric({
  ...
  return (
    <div>
      <span>{label}</span>
      <strong>{value}</strong>
      <small className={good ? 'good' : ''}><OperationalShowcaseIcon name={good ? 'arrow-down' : 'arrow-up'} size={11} />{change}</small>
      <OperationalShowcaseSparkline values={values} color="#6757d5" height={38} />
    </div>
  )
}
