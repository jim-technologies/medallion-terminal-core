  ...
  return (
    <div className="ready-showcase shopify-showcase">
      <header className="shopify-topbar">
        <button className="shopify-mobile-menu" aria-label="Menu"><ReadinessIcon name="menu" /></button>
        <div className="shopify-logo"><span>S</span><strong>shopify</strong></div>
        <label className="shopify-global-search"><ReadinessIcon name="search" size={16} /><input placeholder="Search" /><kbd>⌘ K</kbd></label>
      ...
      <div className="shopify-body">
        <aside className="shopify-sidebar">
          <nav>
            ...
            <button><ReadinessIcon name="chart" size={16} /><span>Analytics</span></button>
            <button><ReadinessIcon name="tag" size={16} /><span>Discounts</span></button>
          </nav>
          <div className="shopify-nav-heading"><span>Sales channels</span><button><ReadinessIcon name="plus" size={14} /></button></div>
          <nav>
            <button><ReadinessIcon name="cart" size={16} /><span>Online Store</span></button>
            <button><ReadinessIcon name="apps" size={16} /><span>Shop</span></button>
          </nav>
          <div className="shopify-nav-heading"><span>Apps</span><button><ReadinessIcon name="plus" size={14} /></button></div>
          <nav><button><ReadinessIcon name="bolt" size={16} /><span>Flow</span></button></nav>
          <div className="shopify-sidebar-footer"><button><ReadinessIcon name="settings" size={16} />Settings</button></div>
        </aside>
          ...
          {section === 'home' && (
            <div className="shopify-content-narrow">
              ...
              <div className="shopify-alert-card">
                ...
                <button className="ready-icon-button" aria-label="Dismiss"><ReadinessIcon name="close" size={15} /></button>
              </div>
              <section className="shopify-home-card">
                <div className="shopify-card-heading">
                  ...
                  <button>Today <ReadinessIcon name="chevron-down" size={13} /></button>
                </div>
                <div className="shopify-metrics">
                  <ShopifyMetric label="Gross sales" value={formatReadinessCurrency(grossSales)} change="+18%" />
                ...
                <div className="shopify-sales-chart">
                  ...
                  <div className="shopify-chart-plot">
                    ...
                    <ReadinessSparkline values={[120, 180, 160, 310, 280, 540, 410, 720, 680, 970, 1140, 1290]} color="#16845b" height={160} />
              ...
              <div className="shopify-home-grid">
                <section className="shopify-home-card">
                  ...
                  <div className="shopify-order-summary">
                    <div><span className="shopify-summary-ring">5</span><span><strong>5 orders</strong><small>{formatReadinessCurrency(grossSales)} total</small></span></div>
          ...
          {section === 'orders' && !showOrder && (
            <div className="shopify-content-wide">
              ...
              <section className="shopify-table-card">
                <div className="shopify-order-tabs"><button className="active">All</button><button>Unfulfilled</button><button>Unpaid</button><button>Open</button><button>Archived</button><button><ReadinessIcon name="plus" size={13} /></button></div>
                <div className="ready-panel-toolbar shopify-table-toolbar">
                  <label className="ready-search-field"><ReadinessIcon name="search" size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search orders" /></label>
                  <button className={fulfillment ? 'active' : ''} onClick={() => setFulfillment(fulfillment ? undefined : 'Unfulfilled')}><ReadinessIcon name="filter" size={14} />Filter</button>
                ...
                <div className="shopify-order-table">
                  ...
                  {filteredOrders.map(order => (
                    <button key={order.id} onClick={() => chooseOrder(order)}>
                      ...
                      <span>{formatReadinessCurrency(order.total, { cents: true })}</span>
                ...
                <div className="shopify-table-footer"><span>Showing {filteredOrders.length} of {orders.length} orders</span><div><button disabled><ReadinessIcon name="chevron-left" size={14} /></button><button><ReadinessIcon name="chevron-right" size={14} /></button></div></div>
              </section>
            </div>
          )}
          ...
          {section === 'orders' && showOrder && selectedOrder && (
            <div className="shopify-content-wide">
              <div className="shopify-order-detail-heading">
                <button className="ready-icon-button" onClick={() => setShowOrder(false)}><ReadinessIcon name="chevron-left" size={18} /></button>
              ...
              <div className="shopify-order-detail-grid">
                <div>
                  <section className="shopify-detail-card">
                    <div className="shopify-detail-card-heading"><div><ReadinessIcon name="package" size={17} /><h2>Unfulfilled</h2><span>West warehouse</span></div><button>•••</button></div>
                    <div className="shopify-line-items">
                      {selectedOrder.items.map(item => (
                        <div key={item.id}>
                          <i style={{ background: item.color }}><ReadinessIcon name="box" size={24} /></i>
                          <span><strong>{item.name}</strong><small>{item.variant}</small><code>{item.sku}</code></span>
                          <em>{formatReadinessCurrency(item.price, { cents: true })} × {item.quantity}</em>
                          <strong>{formatReadinessCurrency(item.price * item.quantity, { cents: true })}</strong>
                        </div>
                      ))}
                    </div>
                  ...
                  <section className="shopify-detail-card">
                    <div className="shopify-detail-card-heading"><div><ReadinessIcon name="money" size={17} /><h2>{selectedOrder.paymentStatus}</h2></div></div>
                    <dl className="shopify-money-summary">
                      <div><dt>Subtotal</dt><dd>{formatReadinessCurrency(selectedOrder.total - 28, { cents: true })}</dd></div>
                      <div><dt>Shipping</dt><dd>$18.00</dd></div>
                      <div><dt>Tax</dt><dd>$10.00</dd></div>
                      <div className="total"><dt>Total</dt><dd>{formatReadinessCurrency(selectedOrder.total, { cents: true })}</dd></div>
                      <div><dt>Paid by customer</dt><dd>{formatReadinessCurrency(selectedOrder.paymentStatus === 'Pending' ? 0 : selectedOrder.total, { cents: true })}</dd></div>
                    </dl>
                  </section>
                  <section className="shopify-detail-card">
          ...
          {section === 'inventory' && (
            <div className="shopify-content-wide">
              ...
              <div className="shopify-inventory-stats">
                <div><span>Inventory value</span><strong>{formatReadinessCurrency(inventory.reduce((sum, item) => sum + item.available * item.price, 0))}</strong><small>Across 2 locations</small></div>
                ...
                <div><span>Sell-through</span><strong>{formatReadinessPercent(0.684)}</strong><small>Last 30 days</small></div>
              </div>
              <section className="shopify-table-card">
                <div className="ready-panel-toolbar shopify-table-toolbar"><label className="ready-search-field"><ReadinessIcon name="search" size={15} /><input placeholder="Search products and SKUs" /></label><button><ReadinessIcon name="filter" size={14} />All locations</button><button><ReadinessIcon name="more" size={15} /></button></div>
                <div className="shopify-inventory-table">
                  ...
                  {inventory.map(item => (
                    <div key={item.id}>
                      ...
                      <span className="shopify-inventory-product"><i style={{ background: item.color }}><ReadinessIcon name="box" size={18} /></i><span><strong>{item.product}</strong><small>{item.variant}</small></span></span>
                      <code>{item.sku}</code>
...
function ShopifyMetric({
  ...
  return <div><span>{label}</span><strong>{value}</strong><small><ReadinessIcon name="arrow-up" size={11} />{change}</small></div>
}
