...
export interface ReadinessIconProps extends SVGProps<SVGSVGElement> {
  name: ReadinessIconName
  size?: number
}
...
export function readinessInitials(name: string): string {
  return name
    .trim()
    .split(/\s+/)
    .slice(0, 2)
...
export function ReadinessAvatar({
  name,
  color = '#4169a1',
  size = 28,
}: {
  ...
  return (
    <span className="ready-avatar" style={style} title={name} aria-label={name}>
      {readinessInitials(name)}
    </span>
  )
}

export function formatReadinessCurrency(
  value: number,
...
export function formatReadinessPercent(value: number, digits = 1): string {
  return new Intl.NumberFormat('en-US', {
    style: 'percent',
