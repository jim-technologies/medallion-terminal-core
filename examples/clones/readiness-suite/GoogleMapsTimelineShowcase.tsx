      ...
      <div className="timeline-body">
        <aside className="timeline-sidebar">
          <div className="timeline-sidebar-heading">
            <button><ReadinessIcon name="chevron-left" size={18} /></button>
            <div><h1>Your Timeline</h1><span>Only you can see your Timeline</span></div>
            <button><ReadinessIcon name="more" size={18} /></button>
          </div>
          <nav>
            {TIMELINE_NAV.map(item => (
              <button key={item.id} className={section === item.id ? 'active' : ''} onClick={() => setSection(item.id)}>
                <ReadinessIcon name={item.icon} size={17} /><span>{item.label}</span>
              </button>
            ))}
          </nav>
          ...
          {section === 'day' && (
            <>
              <div className="timeline-date-picker">
                <button disabled={dayIndex >= days.length - 1} onClick={() => changeDay(1)}><ReadinessIcon name="chevron-left" size={17} /></button>
                <button className="timeline-date-button"><ReadinessIcon name="calendar" size={17} /><span><strong>{day.label}</strong><small>{day.city}</small></span><ReadinessIcon name="chevron-down" size={13} /></button>
                <button disabled={dayIndex <= 0} onClick={() => changeDay(-1)}><ReadinessIcon name="chevron-right" size={17} /></button>
              </div>
                  ...
                  return (
                    <div key={stop.id}>
                      ...
                      <button className={`timeline-stop-card ${stop.id === selectedStop?.id ? 'selected' : ''}`} onClick={() => chooseStop(stop)}>
                        <span className={`timeline-stop-marker ${stop.category.toLowerCase().replace(/\s+/g, '-')}`}><ReadinessIcon name={stop.category === 'Home' ? 'home' : 'location'} size={15} /></span>
                        ...
                        <ReadinessIcon name="more" size={16} />
                      </button>
                    </div>
                  )
                })}
                <div className="timeline-day-complete"><ReadinessIcon name="check" size={14} /><span>Timeline saved to this device</span></div>
              </div>
            </>
          )}
          ...
          {section === 'trips' && (
            <div className="timeline-trips-panel">
              <div className="timeline-period-select"><button>2026 <ReadinessIcon name="chevron-down" size={13} /></button><button>All trip types <ReadinessIcon name="chevron-down" size={13} /></button></div>
              ...
              <button className="timeline-trip-card selected">
                <span className="timeline-trip-cover coast"><ReadinessIcon name="location" size={20} /></span>
              ...
              <button className="timeline-trip-card">
                <span className="timeline-trip-cover forest"><ReadinessIcon name="location" size={20} /></span>
                <span><strong>Point Reyes</strong><small>Jun 27 · 1 day</small><em>5 places · 18 photos</em></span>
              </button>
              <button className="timeline-trip-card">
                <span className="timeline-trip-cover city"><ReadinessIcon name="location" size={20} /></span>
          ...
          {section === 'places' && (
            <div className="timeline-places-panel">
              <label><ReadinessIcon name="search" size={15} /><input placeholder="Search visited places" /></label>
              ...
              {placeGroups.map(([category, stops]) => (
                <section key={category}>
                  <div><span className={`timeline-category-icon ${category.toLowerCase().replace(/\s+/g, '-')}`}><ReadinessIcon name="location" size={14} /></span><h2>{category}</h2><em>{stops.length}</em></div>
                  {stops.map(stop => <button key={stop.id} onClick={() => chooseStop(stop)}><span><strong>{stop.name}</strong><small>{stop.address}</small></span><ReadinessIcon name="chevron-right" size={13} /></button>)}
                </section>
              ))}
            </div>
          )}
        ...
        <main className={`timeline-map ${mapType.toLowerCase()}`}>
          ...
          {day.stops.map((stop, index) => (
            <button
              ...
              title={stop.name}
            >
              <span>{stop.category === 'Home' ? <ReadinessIcon name="home" size={13} /> : index}</span>
          ...
          <div className="timeline-map-controls">
            ...
            <button><ReadinessIcon name="location" size={18} /></button>
          ...
          {selectedStop && (
            <aside className="timeline-place-card">
              <button className="timeline-place-close" onClick={() => setSelectedStopId('')}><ReadinessIcon name="close" size={16} /></button>
              <div className={`timeline-place-hero ${selectedStop.category.toLowerCase().replace(/\s+/g, '-')}`}>
                {selectedStop.media.length > 0
                  ...
                  : <ReadinessIcon name={selectedStop.category === 'Home' ? 'home' : 'location'} size={32} />}
              </div>
              <div className="timeline-place-copy">
                <h2>{selectedStop.name}</h2><span>{selectedStop.category}</span><p>{selectedStop.address}</p>
                <div className="timeline-place-actions"><button><span><ReadinessIcon name="send" size={17} /></span>Directions</button><button><span><ReadinessIcon name="document" size={17} /></span>Details</button><button><span><ReadinessIcon name="more" size={17} /></span>More</button></div>
                <div className="timeline-visit-row"><ReadinessIcon name="clock" size={16} /><span><strong>Visited {selectedStop.arrival} – {selectedStop.departure}</strong><small>{formatTimelineDuration(selectedStop.durationMinutes)} · {day.label}</small></span><ReadinessIcon name="more" size={15} /></div>
...
function TimelineTripRow({ trip }: { trip: TimelineTrip }) {
  return (
    <div className="timeline-trip-row">
      <span><ReadinessIcon name={travelIcon(trip.mode)} size={14} /></span>
      <i />
