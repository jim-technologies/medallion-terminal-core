      ...
      <div className="foundry-body">
        <aside aria-label="Foundry workspace navigation" className="foundry-sidebar">
          ...
          <div className="foundry-sidebar-group">
            <span>Resources</span>
            <button><OperationalShowcaseIcon name="database" size={15} />Datasets</button>
            <button><OperationalShowcaseIcon name="code" size={15} />Code repositories</button>
            <button><OperationalShowcaseIcon name="apps" size={15} />Applications</button>
          </div>
            ...
            <>
              <div className="ready-page-heading foundry-page-heading">
                ...
                <div className="ready-heading-actions">
                  <button className="ready-button secondary"><OperationalShowcaseIcon name="download" size={15} />Export</button>
                  <button className="ready-button primary"><OperationalShowcaseIcon name="plus" size={15} />New object type</button>
                </div>
              </div>
                ...
                {selectedType && (
                  <aside aria-label={`${selectedType.name} inspector`} className="foundry-inspector">
                    ...
                    <div className="foundry-inspector-section">
                      <div className="foundry-inspector-title"><span>Properties</span><button><OperationalShowcaseIcon name="plus" size={13} />Add</button></div>
                      {selectedType.properties.map(property => (
                        <div className="foundry-property" key={property.id}>
                          <OperationalShowcaseIcon name="tag" size={14} />
                    ...
                    <div className="foundry-inspector-section">
                      <div className="foundry-inspector-title"><span>Link types</span><button><OperationalShowcaseIcon name="plus" size={13} />Add</button></div>
                  ...
                  <div className="foundry-object-content">
                    ...
                    <div>
                      ...
                      <div className="foundry-related-grid">
                        {selectedObject.related.map(related => (
                          <button key={related.label}>
                            <span><OperationalShowcaseIcon name="link" size={14} />{related.type}</span>
                      ...
                      <div className="foundry-owner-card">
                        <OperationalShowcaseAvatar name={selectedObject.owner} color="#4f7fb5" size={34} />
                        <span className="foundry-owner-copy"><strong>{selectedObject.owner}</strong><small>Strategic accounts</small></span>
                        <button><OperationalShowcaseIcon name="mail" size={15} /></button>
                      </div>
                    </div>
                  </div>
                </section>
          ...
          {section === 'lineage' && (
            <>
              <div className="ready-page-heading foundry-page-heading">
                ...
                <div className="ready-heading-actions">
                  <button className="ready-button secondary"><OperationalShowcaseIcon name="refresh" size={15} />Refresh</button>
                  <button className="ready-button primary"><OperationalShowcaseIcon name="plus" size={15} />Add resource</button>
                </div>
              </div>
              <div className="foundry-lineage-toolbar">
                <label className="ready-search-field"><OperationalShowcaseIcon name="search" size={15} /><input placeholder="Find resource in graph" /></label>
                <button><OperationalShowcaseIcon name="filter" size={15} />Resource type</button>
                <button><OperationalShowcaseIcon name="activity" size={15} />Health</button>
              ...
              <section className="foundry-lineage-canvas">
                ...
                {lineageNodes.map(node => (
                  ...
                  >
                    <i><OperationalShowcaseIcon name={foundryNodeIcon(node.kind)} size={15} /></i>
                    <span><small>{node.kind}</small><strong>{node.label}</strong><em>{node.detail}</em></span>
                    <b title={node.status} />
                  </button>
                ))}
                <div className="foundry-canvas-controls"><button>+</button><button>−</button><button><OperationalShowcaseIcon name="home" size={14} /></button></div>
                <div className="foundry-lineage-alert"><OperationalShowcaseIcon name="warning" size={16} /><span><strong>Freshness policy breached</strong><small>gold.customer_360 · expected within 10 minutes</small></span><button>Review issue</button></div>
              </section>
            </>
          )}
          ...
          {section === 'actions' && selectedAction && (
            <>
              <div className="ready-page-heading foundry-page-heading">
                ...
                <button className="ready-button primary"><OperationalShowcaseIcon name="plus" size={15} />New action type</button>
              </div>
              <div className="foundry-actions-layout">
                <section className="foundry-action-list">
                  <div className="ready-panel-toolbar"><label className="ready-search-field"><OperationalShowcaseIcon name="search" size={15} /><input placeholder="Search action types" /></label></div>
                  {actions.map(action => (
                    <button
                      key={action.id}
                    ...
                    >
                      <i><OperationalShowcaseIcon name="bolt" size={16} /></i>
                ...
                <section className="foundry-action-detail">
                  <div className="foundry-action-heading">
                    <i><OperationalShowcaseIcon name="bolt" size={20} /></i>
                    ...
                    <div>
                      ...
                      {selectedAction.parameters.map((parameter, index) => (
                        <div className="foundry-parameter" key={parameter}>
                          <span>{index + 1}</span><div><strong>{parameter}</strong><small>{index === 0 ? 'Object reference' : 'Required input'}</small></div><OperationalShowcaseIcon name="more" size={14} />
                        </div>
                      ))}
                    </div>
                    <div>
                      <h3>Submission controls</h3>
                      <div className="foundry-control-card"><OperationalShowcaseIcon name="shield" size={18} /><span><strong>{selectedAction.approval} approval</strong><small>Checked before the action is committed</small></span></div>
                      <div className="foundry-control-card"><OperationalShowcaseIcon name="check" size={18} /><span><strong>Validation enabled</strong><small>All parameter and object rules must pass</small></span></div>
                      <div className="foundry-control-card"><OperationalShowcaseIcon name="activity" size={18} /><span><strong>Audit every run</strong><small>Actor, inputs, result, and side effects retained</small></span></div>
                    </div>
                  </div>
                  <div className="foundry-action-footer"><span><i />Published · version 12</span><button className="ready-button secondary">View run history</button><button className="ready-button primary"><OperationalShowcaseIcon name="bolt" size={14} />Run action</button></div>
                </section>
              </div>
            </>
          )}
