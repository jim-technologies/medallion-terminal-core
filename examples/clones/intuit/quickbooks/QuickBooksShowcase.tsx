    ...
    <header className="quickbooks-topbar">
      ...
      <div className="quickbooks-top-actions">
        ...
        <button className="quickbooks-company-switcher">
          <ProductShowcaseAvatar name={companyName} color="#2ca01c" size={29} />
          <span>{companyName}</span>
          <ProductShowcaseIcon name="chevron-down" size={12} />
        </button>
      </div>
    </header>
  )
    ...
    <aside aria-label="QuickBooks navigation" className="quickbooks-sidebar">
      ...
      <nav aria-label="Business overview">
        <button className={section === 'overview' ? 'is-active' : ''} onClick={() => onSectionChange('overview')}>
          <ProductShowcaseIcon name="home" size={18} /> Home
        </button>
        <button><ProductShowcaseIcon name="sparkles" size={18} /> Business feed <span className="quickbooks-beta">AI</span></button>
        <button className={section === 'cash-flow' ? 'is-active' : ''} onClick={() => onSectionChange('cash-flow')}>
          <ProductShowcaseIcon name="activity" size={18} /> Cash flow
        </button>
        <button><ProductShowcaseIcon name="calendar" size={18} /> Planner</button>
      </nav>
      <div className="quickbooks-sidebar-heading">BOOKMARKS <button aria-label="Add bookmark"><ProductShowcaseIcon name="plus" size={13} /></button></div>
      <nav aria-label="Bookmarks">
        <button className={section === 'transactions' ? 'is-active' : ''} onClick={() => onSectionChange('transactions')}>
          <ProductShowcaseIcon name="bank" size={18} /> Bank transactions
      ...
      <nav aria-label="Intuit applications">
        ...
        <button><ProductShowcaseIcon name="document" size={18} /> Expenses & bills</button>
        <button><ProductShowcaseIcon name="task" size={18} /> Projects</button>
        <button><ProductShowcaseIcon name="team" size={18} /> Payroll</button>
      </nav>
      <footer>
        <button><ProductShowcaseIcon name="apps" size={17} /> Customize</button>
      </footer>
    </aside>
  )
}
...
function QuickBooksCard({
  ...
  return (
    <section className={`quickbooks-card ${className}`}>
      <header>
        <div><h2>{title}</h2><button aria-label={`More ${title} actions`}><ProductShowcaseIcon name="more" size={17} /></button></div>
  ...
  return (
    <div className="quickbooks-overview">
      <header className="quickbooks-page-heading">
        ...
        <button><ProductShowcaseIcon name="grid" size={15} /> Customize layout</button>
      </header>
      <QuickBooksHomeTabs section="overview" onSectionChange={onSectionChange} />
      <section className="quickbooks-business-feed">
        <div className="quickbooks-feed-icon"><ProductShowcaseIcon name="sparkles" size={19} /></div>
        <div>
      ...
      <div className="quickbooks-dashboard-grid">
        <QuickBooksCard title="Cash flow" action="View cash flow" className="quickbooks-cash-card">
          <div className="quickbooks-card-period">LAST 6 MONTHS <ProductShowcaseIcon name="chevron-down" size={12} /></div>
          <div className="quickbooks-cash-summary">
            <div><span>Cash balance</span><strong>{formatProductCurrency(quickBooksCashBalance(cashFlow))}</strong><small><i /> Up {formatProductCurrency(16_620)} this month</small></div>
            <div><span>Money in</span><strong>{formatProductCurrency(cashFlow[cashFlow.length - 1]?.moneyIn ?? 0)}</strong></div>
            <div><span>Money out</span><strong>{formatProductCurrency(cashFlow[cashFlow.length - 1]?.moneyOut ?? 0)}</strong></div>
          </div>
        ...
        <QuickBooksCard title="Tasks" action="View all" className="quickbooks-tasks-card">
          <div className="quickbooks-task-item is-warning">
            <span><ProductShowcaseIcon name="bank" size={17} /></span>
          ...
          <div className="quickbooks-task-item">
            <span><ProductShowcaseIcon name="invoice" size={17} /></span>
            <p><strong>Send {overdue.length} overdue invoice reminder</strong><small>{formatProductCurrency(overdue.reduce((sum, invoice) => sum + invoice.amount, 0))} outstanding</small></p>
            <button>Send</button>
          </div>
          <div className="quickbooks-task-item">
            <span><ProductShowcaseIcon name="check" size={17} /></span>
        ...
        <QuickBooksCard title="Profit and loss" action="View report" className="quickbooks-pnl-card">
          <div className="quickbooks-card-period">THIS MONTH <ProductShowcaseIcon name="chevron-down" size={12} /></div>
          <div className="quickbooks-pnl-total">
            <span>Net profit</span>
            <strong>{formatProductCurrency(46_240)}</strong>
            <small><ProductShowcaseIcon name="arrow-up" size={12} /> 12.4% from last month</small>
          </div>
          <div className="quickbooks-pnl-bars">
            <div>
              <span>Income</span>
              <i><b style={{ width: '92%' }} /></i>
              <strong>{formatProductCurrency(151_020)}</strong>
            </div>
            <div>
              <span>Expenses</span>
              <i><b style={{ width: '64%' }} /></i>
              <strong>{formatProductCurrency(104_780)}</strong>
            </div>
          </div>
        </QuickBooksCard>
        <QuickBooksCard title="Invoices" action="View invoices" className="quickbooks-invoice-card">
          <div className="quickbooks-card-period">LAST 365 DAYS <ProductShowcaseIcon name="chevron-down" size={12} /></div>
          <div className="quickbooks-invoice-total">
            <span>Open invoices</span>
            <strong>{formatProductCurrency(openTotal)}</strong>
          </div>
          ...
          <dl className="quickbooks-invoice-legend">
            <div><dt><i className="is-overdue" /> Overdue</dt><dd>{formatProductCurrency(overdue.reduce((sum, invoice) => sum + invoice.amount, 0))}</dd></div>
            <div><dt><i className="is-open" /> Not due yet</dt><dd>{formatProductCurrency(open.filter(invoice => invoice.status === 'Open').reduce((sum, invoice) => sum + invoice.amount, 0))}</dd></div>
            <div><dt><i className="is-paid" /> Paid last 30 days</dt><dd>{formatProductCurrency(invoices.filter(invoice => invoice.status === 'Paid').reduce((sum, invoice) => sum + invoice.amount, 0))}</dd></div>
          </dl>
        </QuickBooksCard>
        <QuickBooksCard title="Expenses" action="View expenses" className="quickbooks-expense-card">
          ...
          <div className="quickbooks-expense-content">
            <div className="quickbooks-donut"><span>{formatProductCurrency(104_780, { compact: true })}<small>Total</small></span></div>
            <dl>
              <div><dt><i className="is-payroll" /> Payroll</dt><dd>{formatProductCurrency(48_320)}</dd></div>
              <div><dt><i className="is-software" /> Software</dt><dd>{formatProductCurrency(21_840)}</dd></div>
              <div><dt><i className="is-marketing" /> Marketing</dt><dd>{formatProductCurrency(18_400)}</dd></div>
              <div><dt><i className="is-other" /> Other</dt><dd>{formatProductCurrency(16_220)}</dd></div>
            </dl>
          </div>
        </QuickBooksCard>
        <QuickBooksCard title="Bank accounts" action="Go to bank transactions" className="quickbooks-bank-card">
          ...
          <button className="quickbooks-connect-link"><ProductShowcaseIcon name="plus" size={14} /> Connect another account</button>
        </QuickBooksCard>
      </div>
    </div>
  )
  ...
  return (
    <div className="quickbooks-cash-flow-page">
      <header className="quickbooks-page-heading">
        ...
        <button><ProductShowcaseIcon name="settings" size={15} /> Cash flow settings</button>
      </header>
      <QuickBooksHomeTabs section="cash-flow" onSectionChange={onSectionChange} />
      <div className="quickbooks-cash-kpis">
        <article><span>Cash balance</span><strong>{formatProductCurrency(latest?.balance ?? 0)}</strong><small className="is-positive"><ProductShowcaseIcon name="arrow-up" size={12} /> 10.5% this month</small></article>
        <article><span>Money in</span><strong>{formatProductCurrency(latest?.moneyIn ?? 0)}</strong><small>12 transactions</small></article>
        <article><span>Money out</span><strong>{formatProductCurrency(latest?.moneyOut ?? 0)}</strong><small>38 transactions</small></article>
        <article><span>Projected in 90 days</span><strong>{formatProductCurrency(214_900)}</strong><small className="is-positive">Healthy runway</small></article>
      </div>
        ...
        <QuickBooksCard title="Upcoming" action="Open planner">
          {[
            ...
            <div className="quickbooks-upcoming-row" key={String(label)}>
              ...
              <strong className={direction === 'in' ? 'is-in' : 'is-out'}>{formatProductCurrency(Number(amount))}</strong>
            </div>
          ))}
        </QuickBooksCard>
        <QuickBooksCard title="Cash flow insights">
          <div className="quickbooks-insight">
            <span><ProductShowcaseIcon name="sparkles" size={17} /></span>
          ...
          <div className="quickbooks-insight">
            <span><ProductShowcaseIcon name="chart" size={17} /></span>
  ...
  return (
    <div className="quickbooks-transactions-page">
      <header className="quickbooks-page-heading">
        ...
        <div><button><ProductShowcaseIcon name="arrow-down" size={14} /> Update</button><button><ProductShowcaseIcon name="plus" size={14} /> Link account</button></div>
      </header>
      <div className="quickbooks-account-strip">
        <button className="is-active">
          <span className="quickbooks-bank-logo">NW</span>
          <span><strong>Operating checking</strong><small>Bank balance {formatProductCurrency(174_620)}</small></span>
          <b>4</b>
        </button>
        <button>
          <span className="quickbooks-bank-logo is-card">V</span>
        ...
        <button className="quickbooks-add-account"><ProductShowcaseIcon name="plus" size={16} /> Add account</button>
      </div>
      ...
      <div className="quickbooks-transaction-toolbar">
        <label><ProductShowcaseIcon name="search" size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search transactions" /></label>
        <div>
          <button><ProductShowcaseIcon name="filter" size={14} /> Filter</button>
          <button><ProductShowcaseIcon name="settings" size={14} /> Table settings</button>
          <button>Batch actions <ProductShowcaseIcon name="chevron-down" size={12} /></button>
        </div>
      </div>
          ...
          <tbody>
            {filtered.map(transaction => (
              <tr key={transaction.id}>
                ...
                <td className={transaction.amount >= 0 ? 'is-in' : 'is-out'}>{formatProductCurrency(transaction.amount, { maximumFractionDigits: 2 })}</td>
                <td>
                  {transaction.suggestion && <span className="quickbooks-ai-suggestion"><ProductShowcaseIcon name="sparkles" size={12} /> Suggested</span>}
                  <button className="quickbooks-category-select">{transaction.suggestion ?? transaction.category}<ProductShowcaseIcon name="chevron-down" size={12} /></button>
