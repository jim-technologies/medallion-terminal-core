    ...
    <>
      <header className="hubspot-topbar">
        ...
        <label className="hubspot-global-search">
          <SmeCloneIcon name="search" size={15} />
          <input
            value={query}
        ...
        <div className="hubspot-global-actions">
          <button aria-label="Marketplace"><SmeCloneIcon name="apps" /></button>
          <button aria-label="Calling"><SmeCloneIcon name="phone" /></button>
      ...
      <nav className="hubspot-main-nav" aria-label="HubSpot sections">
        {['CRM', 'Marketing', 'Content', 'Sales', 'Service', 'Automation', 'Reporting', 'Commerce', 'Data Management'].map((item, index) => (
          <button className={index === 0 ? 'is-active' : ''} key={item}>
            {item}
            {index < 6 && <SmeCloneIcon name="chevron-down" size={11} />}
          </button>
        ))}
      </nav>
    </>
    ...
    <aside className="hubspot-section-nav">
      ...
      <nav>
        <button className={section === 'contacts' || section === 'record' ? 'is-active' : ''} onClick={() => onSectionChange('contacts')}>
          <SmeCloneIcon name="contact" size={17} /> Contacts
        </button>
        <button><SmeCloneIcon name="company" size={17} /> Companies</button>
        <button className={section === 'pipeline' ? 'is-active' : ''} onClick={() => onSectionChange('pipeline')}>
          <SmeCloneIcon name="money" size={17} /> Deals
        </button>
        <button><SmeCloneIcon name="task" size={17} /> Tickets</button>
        <button><SmeCloneIcon name="document" size={17} /> Lists</button>
      </nav>
      <div className="hubspot-section-subtitle">My workspace</div>
      <nav>
        <button><SmeCloneIcon name="task" size={17} /> Tasks</button>
        <button><SmeCloneIcon name="activity" size={17} /> Activity feed</button>
        <button><SmeCloneIcon name="reports" size={17} /> Forecast</button>
      </nav>
      <button className="hubspot-breeze-card">
        <span><SmeCloneIcon name="sparkles" size={16} /></span>
  ...
  return (
    <div className="hubspot-index-page">
      <header className="hubspot-page-heading">
        <div>
          <div className="hubspot-breadcrumb">CRM <SmeCloneIcon name="chevron-right" size={12} /> Contacts</div>
          <h1>Contacts</h1>
          <p>{contacts.length} records · Updated a few seconds ago</p>
        </div>
        <div>
          <button className="hubspot-secondary-button">Import</button>
          <button className="hubspot-secondary-button">Actions <SmeCloneIcon name="chevron-down" size={12} /></button>
          <button className="hubspot-primary-button"><SmeCloneIcon name="plus" size={15} /> Create contact</button>
        </div>
      </header>
      <div className="hubspot-saved-views">
        ...
        <button><SmeCloneIcon name="plus" size={13} /> Add view</button>
      </div>
      <div className="hubspot-filter-row">
        <button>Contact owner <SmeCloneIcon name="chevron-down" size={12} /></button>
        <label>
        ...
        <button>Create date <SmeCloneIcon name="chevron-down" size={12} /></button>
        <button>Last activity date <SmeCloneIcon name="chevron-down" size={12} /></button>
        <button className="hubspot-more-filters"><SmeCloneIcon name="filter" size={14} /> Advanced filters</button>
      </div>
      <div className="hubspot-table-actions">
        <span><input type="checkbox" aria-label="Select all contacts" /> {contacts.length} contacts</span>
        <div>
          <button><SmeCloneIcon name="sort" size={14} /> Sort</button>
          <button><SmeCloneIcon name="table" size={14} /> Edit columns</button>
          <button aria-label="More table actions"><SmeCloneIcon name="more" size={16} /></button>
        </div>
      </div>
        ...
        <table className="hubspot-contact-table">
          <thead>
            <tr>
              ...
              <th>Contact name <SmeCloneIcon name="arrow-up" size={12} /></th>
              <th>Email</th>
              <th>Company name</th>
              <th>Lead status</th>
              <th>Contact owner</th>
              ...
              <tr key={contact.id} onClick={() => onSelect(contact)}>
                ...
                <td>
                  <button className="hubspot-contact-name">
                    <SmeCloneAvatar name={contact.name} color={contact.ownerColor} size={30} />
                ...
                <td><span className="hubspot-owner"><SmeCloneAvatar name={contact.owner} color={contact.ownerColor} size={21} /> {contact.owner}</span></td>
      ...
      <footer className="hubspot-pagination">
        ...
        <button disabled><SmeCloneIcon name="chevron-left" size={14} /></button>
        <button disabled><SmeCloneIcon name="chevron-right" size={14} /></button>
      </footer>
    </div>
  )
}
    ...
    : kind === 'call'
      ...
      : kind === 'meeting'
        ...
        : kind === 'task'
          ? 'task'
          : 'note'
  return <span className={`hubspot-activity-icon is-${kind}`}><SmeCloneIcon name={icon} size={15} /></span>
}
...
function HubSpotRecord({
  ...
  const quickActions: Array<[SmeCloneIconName, string]> = [
    ['note', 'Note'],
    ['mail', 'Email'],
    ['phone', 'Call'],
    ['task', 'Task'],
  ...
  return (
    <div className="hubspot-record-page">
      <div className="hubspot-record-breadcrumb">
        <button onClick={onBack}><SmeCloneIcon name="chevron-left" size={14} /> Contacts</button>
        <span>/</span>
        <strong>{contact.name}</strong>
        <div>
          <button>Actions <SmeCloneIcon name="chevron-down" size={12} /></button>
          <button aria-label="Previous contact"><SmeCloneIcon name="chevron-left" size={14} /></button>
          <button aria-label="Next contact"><SmeCloneIcon name="chevron-right" size={14} /></button>
        </div>
      </div>
      <div className="hubspot-record-columns">
        <aside className="hubspot-record-left">
          <div className="hubspot-contact-hero">
            <SmeCloneAvatar name={contact.name} color={contact.ownerColor} size={54} />
            <h1>{contact.name}</h1>
          ...
          <div className="hubspot-quick-actions">
            {quickActions.map(([icon, label]) => (
              <button key={label}><span><SmeCloneIcon name={icon} size={15} /></span>{label}</button>
            ))}
          </div>
          <section className="hubspot-record-card hubspot-about-card">
            <header><strong>About this contact</strong><button><SmeCloneIcon name="settings" size={14} /></button></header>
            <dl>
        ...
        <main className="hubspot-record-middle">
          ...
          <section className="hubspot-upcoming-card">
            <header><strong>Upcoming</strong><button>All activities <SmeCloneIcon name="chevron-down" size={12} /></button></header>
            <div>
              <span className="hubspot-activity-icon is-task"><SmeCloneIcon name="task" size={15} /></span>
          ...
          <div className="hubspot-activity-controls">
            <label><SmeCloneIcon name="search" size={14} /><input placeholder="Search activities" /></label>
            <button>Filter activity <SmeCloneIcon name="chevron-down" size={12} /></button>
              ...
              <article key={activity.id}>
                ...
                <div>
                  <header>
                    ...
                    <button aria-label="More activity actions"><SmeCloneIcon name="more" size={16} /></button>
                  </header>
                  <p>{activity.body}</p>
        ...
        <aside className="hubspot-record-right">
          <section className="hubspot-record-card">
            <header><strong>Company (1)</strong><button><SmeCloneIcon name="plus" size={14} /> Add</button></header>
            <div className="hubspot-association">
              <span className="hubspot-company-icon"><SmeCloneIcon name="company" size={19} /></span>
              <p><strong>{contact.company}</strong><small>{contact.city}</small><small>{formatSmeCurrency(contact.annualRevenue, { compact: true })} annual revenue</small></p>
            </div>
          </section>
          <section className="hubspot-record-card">
            <header><strong>Deals ({relatedDeals.length})</strong><button><SmeCloneIcon name="plus" size={14} /> Add</button></header>
            {relatedDeals.length > 0 ? relatedDeals.map(deal => (
              <div className="hubspot-deal-association" key={deal.id}>
                <strong>{deal.name}</strong>
                <span>{formatSmeCurrency(deal.amount)} · {deal.stage}</span>
                <small>Close date {deal.closeDate}</small>
              </div>
            )) : <p className="hubspot-card-empty">No associated deals</p>}
          </section>
          <section className="hubspot-record-card">
            <header><strong>Attachments</strong><button><SmeCloneIcon name="plus" size={14} /> Add</button></header>
            <div className="hubspot-attachment"><SmeCloneIcon name="document" size={18} /><span><strong>Operating brief.pdf</strong><small>2.4 MB · Yesterday</small></span></div>
            <div className="hubspot-attachment"><SmeCloneIcon name="document" size={18} /><span><strong>Discovery notes.docx</strong><small>184 KB · Jul 16</small></span></div>
          </section>
          <section className="hubspot-breeze-summary">
            <header><SmeCloneIcon name="sparkles" size={16} /><strong>Breeze summary</strong></header>
...
function HubSpotPipeline({
  ...
  return (
    <div className="hubspot-pipeline-page">
      <header className="hubspot-page-heading">
        <div>
          <div className="hubspot-breadcrumb">CRM <SmeCloneIcon name="chevron-right" size={12} /> Deals</div>
          <h1>Sales pipeline</h1>
          <p>{deals.length} open and closed deals · Weighted value {formatSmeCurrency(hubSpotWeightedPipeline(deals), { compact: true })}</p>
        </div>
        <div>
          <button className="hubspot-secondary-button">Pipeline actions <SmeCloneIcon name="chevron-down" size={12} /></button>
          <button className="hubspot-primary-button"><SmeCloneIcon name="plus" size={15} /> Create deal</button>
        </div>
      </header>
      <div className="hubspot-pipeline-toolbar">
        <div>
          <button>Sales pipeline <SmeCloneIcon name="chevron-down" size={12} /></button>
          <button>All deals <SmeCloneIcon name="chevron-down" size={12} /></button>
          <button>Deal owner <SmeCloneIcon name="chevron-down" size={12} /></button>
        </div>
        <div>
          <button><SmeCloneIcon name="board" size={14} /> Board</button>
          <button><SmeCloneIcon name="table" size={14} /> Table</button>
        </div>
      </div>
          ...
          return (
            <section key={stage}>
              <header>
                ...
                <small>{formatSmeCurrency(total, { compact: true })}</small>
              </header>
              <div>
                {stageDeals.map(deal => (
                  <button className="hubspot-deal-card" key={deal.id} onClick={() => onOpenContact(deal.contactId)}>
                    ...
                    <b>{formatSmeCurrency(deal.amount)}</b>
                    <dl>
                      <div><dt>Close date</dt><dd>{deal.closeDate}</dd></div>
                      <div><dt>Probability</dt><dd>{Math.round(deal.probability * 100)}%</dd></div>
                    </dl>
                    <footer><SmeCloneAvatar name={deal.owner} color={deal.ownerColor} size={23} /> {deal.owner}</footer>
                  </button>
                ))}
                <button className="hubspot-add-deal"><SmeCloneIcon name="plus" size={14} /> Add deal</button>
              </div>
            </section>
          )
        })}
