  ...
  return (
    <header className="airtable-topbar">
      ...
      <label className="airtable-global-search">
        ...
        <span className="sme-sr-only">Search workspace</span>
        <input
          value={query}
    ...
    <div className="airtable-basebar">
      ...
      <div className="airtable-base-actions">
        <span className="airtable-presence">
          <SmeCloneAvatar name="Maya Chen" color="#7c5ce7" size={24} />
          <SmeCloneAvatar name="Noah Williams" color="#1677c8" size={24} />
          <SmeCloneAvatar name="Lina Ortiz" color="#be4c8d" size={24} />
        </span>
        <button className="airtable-share-button"><SmeCloneIcon name="share" size={15} /> Share</button>
      </div>
    </div>
  )
}
...
function AirtableSidebar({
  ...
  return (
    <aside className="airtable-sidebar">
      <button className="airtable-create-button"><SmeCloneIcon name="plus" size={16} /> Create</button>
      <div className="airtable-sidebar-heading">
        <span>Views</span>
        <div>
          <button aria-label="Search views"><SmeCloneIcon name="search" size={14} /></button>
          <button aria-label="Add view"><SmeCloneIcon name="plus" size={14} /></button>
        </div>
      </div>
      <nav className="airtable-view-list">
        {navItems.map(item => (
          ...
          >
            <SmeCloneIcon name={item.icon} size={15} />
            <span>{item.label}</span>
          </button>
        ))}
      </nav>
      <div className="airtable-sidebar-group">
        <button><SmeCloneIcon name="chevron-down" size={13} /> Team views</button>
        <span>My priorities</span>
        <span>Launch calendar</span>
        <span>Budget tracker</span>
      </div>
      <div className="airtable-sidebar-bottom">
        <button><SmeCloneIcon name="apps" size={15} /> Extensions</button>
        <button><SmeCloneIcon name="document" size={15} /> Base guide</button>
      </div>
    </aside>
  )
}
  ...
  return (
    <div className="airtable-view-toolbar">
      <div className="airtable-view-title">
        <SmeCloneIcon name={view === 'board' ? 'board' : view === 'record' ? 'grid' : 'table'} size={16} />
        <strong>
      ...
      <div className="airtable-toolbar-actions">
        <label className="airtable-status-filter">
          <SmeCloneIcon name="filter" size={14} />
          <span>Status</span>
          <select
            value={status}
        ...
        <button className="airtable-toolbar-hide-mobile"><SmeCloneIcon name="grid" size={14} /> Color</button>
        <button className="airtable-toolbar-hide-mobile"><SmeCloneIcon name="share" size={14} /> Share view</button>
        <button aria-label="More view actions"><SmeCloneIcon name="more" size={16} /></button>
      </div>
    </div>
  )
}
      ...
      <table className="airtable-grid-table">
        <thead>
          <tr>
            ...
            <th><SmeCloneIcon name="contact" size={13} /> Owner</th>
            <th><span>◎</span> Priority</th>
            <th><SmeCloneIcon name="calendar" size={13} /> Due date</th>
            <th><span>$</span> Budget</th>
            <th><span>%</span> Progress</th>
            <th><SmeCloneIcon name="activity" size={13} /> Last modified</th>
            <th className="airtable-add-column"><SmeCloneIcon name="plus" size={14} /></th>
          </tr>
        </thead>
        <tbody>
            ...
            >
              ...
              <td>
                <span className="airtable-owner">
                  <SmeCloneAvatar name={record.owner} color={record.ownerColor} size={22} />
                  {record.owner}
                </span>
              </td>
              <td><AirtablePriority priority={record.priority} /></td>
              <td>{record.dueDate}</td>
              <td>{formatSmeCurrency(record.budget)}</td>
              <td>
                <span className="airtable-progress">
                  <span><i style={{ width: formatSmePercent(record.progress) }} /></span>
                  {formatSmePercent(record.progress)}
                </span>
              </td>
          ...
          <tr className="airtable-add-row">
            <td className="airtable-row-number"><SmeCloneIcon name="plus" size={13} /></td>
        ...
        return (
          <section className="airtable-board-lane" key={status}>
            <header>
              ...
              <button aria-label={`More ${status} actions`}><SmeCloneIcon name="more" size={16} /></button>
            </header>
            <div className="airtable-lane-budget">
              <span>Total budget</span>
              <strong>{formatSmeCurrency(laneTotal, { compact: true })}</strong>
            </div>
            <div className="airtable-board-cards">
              {laneRecords.map(record => (
                <button className="airtable-board-card" key={record.id} onClick={() => onSelect(record)}>
                  ...
                  <footer>
                    <SmeCloneAvatar name={record.owner} color={record.ownerColor} size={23} />
                    <span>{record.dueDate}</span>
                    <span>◌ {record.comments ?? 0}</span>
                  </footer>
                </button>
              ))}
              <button className="airtable-add-card"><SmeCloneIcon name="plus" size={14} /> Add record</button>
            </div>
          </section>
        )
      })}
    ...
    <div className="airtable-record-review">
      <aside className="airtable-record-list">
        ...
        <div className="airtable-record-list-heading">
          ...
          <button><SmeCloneIcon name="filter" size={14} /></button>
        </div>
        {records.map(record => (
          <button
          ...
          >
            ...
            <SmeCloneIcon name="chevron-right" size={14} />
          </button>
        ))}
      </aside>
      {active ? (
        <article className="airtable-record-detail">
          <header className="airtable-record-detail-header">
            ...
            <div>
              <button className="airtable-record-action"><SmeCloneIcon name="check" size={15} /> Mark complete</button>
              <button className="airtable-icon-button" aria-label="More record actions"><SmeCloneIcon name="more" /></button>
            </div>
          </header>
          <div className="airtable-record-body">
            <section className="airtable-record-fields">
              ...
              <dl>
                ...
                <div>
                  <dt>Owner</dt>
                  <dd><SmeCloneAvatar name={active.owner} color={active.ownerColor} size={23} /> {active.owner}</dd>
                </div>
                <div><dt>Priority</dt><dd><AirtablePriority priority={active.priority} /></dd></div>
                <div><dt>Due date</dt><dd>{active.dueDate}</dd></div>
                <div><dt>Budget</dt><dd>{formatSmeCurrency(active.budget)}</dd></div>
                <div><dt>Progress</dt><dd>{formatSmePercent(active.progress)}</dd></div>
              </dl>
            ...
            <aside className="airtable-comments">
              ...
              <div className="airtable-comment">
                <SmeCloneAvatar name="Maya Chen" color="#7c5ce7" size={26} />
              ...
              <div className="airtable-comment">
                <SmeCloneAvatar name={active.owner} color={active.ownerColor} size={26} />
              ...
              <label className="airtable-comment-input">
                ...
                <button aria-label="Send comment"><SmeCloneIcon name="arrow-up" size={14} /></button>
              </label>
              <button className="airtable-revision-link"><SmeCloneIcon name="activity" size={14} /> View revision history</button>
            </aside>
          </div>
        </article>
      ...
      <div className="airtable-app-shell">
        ...
        <main className="airtable-main">
          <div className="airtable-table-tabs">
            ...
            <button><SmeCloneIcon name="plus" size={14} /> Add or import</button>
          </div>
