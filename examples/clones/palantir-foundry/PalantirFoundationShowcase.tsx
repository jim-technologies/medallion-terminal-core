    ...
    <div className="ready-showcase foundry-showcase palantir-foundation-showcase">
      <header className="foundry-topbar">
        ...
        <div className="ready-top-actions">
          ...
          <OperationalShowcaseAvatar name={CLONE_DEMO_IDENTITY.user} color="#496f9a" size={27} />
        </div>
      </header>
        ...
        <aside className="foundry-sidebar">
          ...
          <nav>
            {FOUNDATION_NAV.map(item => (
              ...
              >
                <OperationalShowcaseIcon name={item.icon} size={16} />
          ...
          <div className="foundry-sidebar-group palantir-covered-group">
            ...
            {[
              ...
              <div className="palantir-covered-link" key={label}>
                ...
                <strong>{label}</strong>
                <OperationalShowcaseIcon name="check" size={12} />
              </div>
            ))}
          </div>
          <div className="foundry-sidebar-footer">
            <OperationalShowcaseIcon name="shield" size={15} />
          ...
          <article className="palantir-capability-card" key={capability.id}>
            <header>
              <span className="palantir-capability-icon">
                <OperationalShowcaseIcon name={capabilityIcon(capability.id)} size={17} />
              </span>
            ...
            <footer><OperationalShowcaseIcon name="shield" size={12} /><span>Host: {capability.hostBoundary}</span></footer>
          </article>
        ))}
      </div>

      <section className="palantir-host-boundary">
        <span><OperationalShowcaseIcon name="shield" size={20} /></span>
    ...
    <>
      <div className="ready-page-heading foundry-page-heading">
        ...
        <div className="ready-heading-actions">
          <button className="ready-button secondary"><OperationalShowcaseIcon name="arrow-up" size={14} />Upload</button>
          <button className="ready-button primary"><OperationalShowcaseIcon name="plus" size={14} />New resource</button>
        </div>
      </div>
      ...
      <div className="palantir-quick-filters">
        <button><span><OperationalShowcaseIcon name="layers" size={17} /></span><div><strong>{CLONE_DEMO_IDENTITY.company}</strong><small>7 active resources</small></div><OperationalShowcaseIcon name="chevron-right" size={14} /></button>
        <button><span><OperationalShowcaseIcon name="people" size={17} /></span><div><strong>Shared with you</strong><small>{resources.filter(resource => resource.shared).length} resources</small></div><OperationalShowcaseIcon name="chevron-right" size={14} /></button>
        <button><span><OperationalShowcaseIcon name="check" size={17} /></span><div><strong>Promoted items</strong><small>{resources.filter(resource => resource.pinned).length} pinned resources</small></div><OperationalShowcaseIcon name="chevron-right" size={14} /></button>
      </div>
      ...
      <div className="palantir-compass-layout">
        ...
        <section className="palantir-compass-table-panel">
          <div className="ready-panel-toolbar">
            <label className="ready-search-field"><OperationalShowcaseIcon name="search" size={15} /><input value={query} onChange={event => onQueryChange(event.target.value)} placeholder="Search files and resources" /></label>
            <button><OperationalShowcaseIcon name="filter" size={14} />Filters</button>
            <button aria-label="More options"><OperationalShowcaseIcon name="more" size={15} /></button>
          </div>
          <div className="palantir-compass-table">
            ...
            {filteredResources.map(resource => (
              <button key={resource.id} className={selectedResource?.id === resource.id ? 'selected' : ''} onClick={() => onSelectResource(resource)}>
                <span className="palantir-resource-name"><i><OperationalShowcaseIcon name={resourceIcon(resource.kind)} size={15} /></i><span><strong>{resource.name}</strong><small>{resource.description}</small></span>{resource.pinned && <OperationalShowcaseIcon name="flag" size={12} />}</span>
        ...
        {selectedResource && (
          <aside className="palantir-resource-detail">
            <header><span><OperationalShowcaseIcon name={resourceIcon(selectedResource.kind)} size={20} /></span><button><OperationalShowcaseIcon name="close" size={14} /></button></header>
            <h2>{selectedResource.name}</h2><small>{selectedResource.kind}</small>
            <PalantirResourceStatus status={selectedResource.status} />
            <p>{selectedResource.description}</p>
            <div className="palantir-resource-actions"><button>Open</button><button><OperationalShowcaseIcon name="more" size={14} /></button></div>
            <dl>
    ...
    <>
      ...
      <div className="palantir-data-layout">
        <section className="palantir-connection-list">
          <header><div><h2>Sources</h2><span>Connection health</span></div><button><OperationalShowcaseIcon name="refresh" size={14} /></button></header>
          {connections.map(connection => (
            <button key={connection.id} className={connection.id === selectedConnection.id ? 'selected' : ''} onClick={() => onSelectConnection(connection)}>
              <i><OperationalShowcaseIcon name={connection.sourceType.includes('S3') ? 'document' : 'database'} size={16} /></i>
        ...
        <section className="palantir-connection-detail">
          <header>
            <i><OperationalShowcaseIcon name="database" size={19} /></i>
          ...
          <div className="palantir-connection-capabilities">
            <h3>Capabilities</h3>
            {['Batch sync', 'Streaming sync', 'Webhook', 'Dataset export'].map((capability, index) => <div key={capability}><OperationalShowcaseIcon name={index < 3 ? 'check' : 'arrow-up'} size={14} /><span><strong>{capability}</strong><small>{index < 3 ? 'Configured and healthy' : 'Available on demand'}</small></span><em>{index < 3 ? 'Active' : 'Ready'}</em></div>)}
          </div>
        </section>
      </div>
      ...
      <section className="palantir-pipeline-panel">
        ...
        <footer><span><i />All schema checks passed</span><span>Next scheduled build · 9:10 AM</span><button>Open lineage <OperationalShowcaseIcon name="chevron-right" size={12} /></button></footer>
      </section>
    </>
  )
}
  ...
  return (
    <>
      <div className="ready-page-heading foundry-page-heading">
        ...
        <div className="ready-heading-actions"><button className="ready-button secondary">Open in desktop</button><button className="ready-button primary"><OperationalShowcaseIcon name="plus" size={14} />New branch</button></div>
      </div>
      ...
      <section className="palantir-repository-shell">
        ...
        <div className="palantir-repository-toolbar">
          <button><OperationalShowcaseIcon name="graph" size={14} /><span>main</span><OperationalShowcaseIcon name="chevron-down" size={12} /></button>
          <span>Protected branch · all checks required</span>
          <div><button><OperationalShowcaseIcon name="bolt" size={14} />Preview</button><button><OperationalShowcaseIcon name="check" size={14} />Test</button><button className="primary">Propose changes</button></div>
        </div>
        <div className="palantir-repository-body">
          <aside className="palantir-repository-tree">
            <header><strong>Explorer</strong><button><OperationalShowcaseIcon name="more" size={14} /></button></header>
            {entries.map(entry => (
              <button
                key={entry.path}
              ...
              >
                <OperationalShowcaseIcon name={entry.kind === 'folder' ? 'chevron-right' : entry.language === 'Markdown' ? 'document' : 'code'} size={13} />
          ...
          <section className="palantir-code-editor">
            <header><span><OperationalShowcaseIcon name="code" size={13} />{selectedEntry.path}</span><div><small>{selectedEntry.language ?? 'Folder'}</small><button><OperationalShowcaseIcon name="more" size={14} /></button></div></header>
          ...
          <aside className="palantir-repository-helper">
            ...
            <section>
              ...
              {['Type check', 'Unit tests · 18', 'Output schema'].map(check => <div className="palantir-check-row" key={check}><OperationalShowcaseIcon name="check" size={13} /><span><strong>{check}</strong><small>Passed on main</small></span></div>)}
            </section>
            <section className="palantir-repository-proposal"><OperationalShowcaseIcon name="graph" size={16} /><div><strong>1 open proposal</strong><span>feature/customer-risk · ready for review</span></div><button>View</button></section>
          </aside>
        </div>
...
function PipelineNode({
  ...
  return <button className={`palantir-pipeline-node ${active ? 'active' : ''}`}><i><OperationalShowcaseIcon name={icon} size={15} /></i><span><small>{kind}</small><strong>{label}</strong><em>{detail}</em></span><b /></button>
}
...
function capabilityIcon(id: string): OperationalShowcaseIconName {
...
function resourceIcon(kind: PalantirCompassResourceKind): OperationalShowcaseIconName {
