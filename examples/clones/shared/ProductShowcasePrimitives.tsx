...
// Neutral helpers shared by product-specific business application showcases.
export type ProductShowcaseIconName =
  | 'activity'
  | 'apps'
  | 'arrow-down'
  | 'arrow-up'
...
export interface ProductShowcaseIconProps extends SVGProps<SVGSVGElement> {
  name: ProductShowcaseIconName
  size?: number
}
 ...
 */
export function ProductShowcaseIcon({
  name,
  size = 18,
  ...props
...
export interface ProductShowcaseAvatarProps {
  name: string
  color?: string
  size?: number
  title?: string
...
export function productShowcaseInitials(name: string): string {
  return name
    .trim()
    .split(/\s+/)
    .slice(0, 2)
...
export function ProductShowcaseAvatar({
  name,
  color = '#516174',
  size = 28,
  title,
}: ProductShowcaseAvatarProps) {
  return (
    <span
      className="product-showcase-avatar"
      style={{ '--avatar-color': color, '--avatar-size': `${size}px` } as CSSProperties}
      title={title ?? name}
      aria-label={name}
    >
      {productShowcaseInitials(name)}
    </span>
  )
}
...
export function formatProductPercent(value: number, digits = 0): string {
  return new Intl.NumberFormat('en-US', {
    style: 'percent',
