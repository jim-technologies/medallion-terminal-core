...
var o = ((e = "") => {
	...
	delLDelim: xe,
	delRDelim: Se,
	url: a(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", N).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),
	_backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,
	del: /^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,
	text: a(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol", N).getRegex()
}, Ae = {
	...F,
	br: a(C).replace("{2,}", "*").getRegex(),
	text: a(F.text).replace("\\b_", "\\b_| {2,}\\n").replace(/\{2,\}/g, "*").getRegex()
}, I = {
	normal: x,
	gfm: se,
	pedantic: ce
...
function z(e, t) {
	if (t) {
		if (s.escapeTest.test(e)) return e.replace(s.escapeReplace, R);
	} else if (s.escapeTestNoEncode.test(e)) return e.replace(s.escapeReplaceNoEncode, R);
	return e;
}
function B(e) {
	try {
...
var G = class {
	...
	code(e) {
		...
		if (t) {
			let e = this.options.pedantic ? t[0] : U(t[0]);
			return {
				type: "code",
				raw: e,
...
${c}` : c;
	...
	constructor(e) {
		...
		this.options.pedantic ? (n.block = I.pedantic, n.inline = L.pedantic) : this.options.gfm && (n.block = I.gfm, this.options.breaks ? n.inline = L.breaks : n.inline = L.gfm), this.tokenizer.rules = n;
	}
	static get rules() {
		return {
			block: I,
...
`;
	...
	link({ href: e, title: t, tokens: n }) {
		let r = this.parser.parseInline(n), i = B(e);
		if (i === null) return r;
		e = i;
		let a = "<a href=\"" + e + "\"";
...
}, $.parseInline = Q.parseInline, $.Parser = Y, $.parser = Y.parse, $.Renderer = q, $.TextRenderer = J, $.Lexer = K, $.lexer = K.lex, $.Tokenizer = G, $.Hooks = X, $.parse = $;
var Fe = $.options, Ie = $.setOptions, Le = $.use, Re = $.walkTokens, ze = $.parseInline, Be = $, Ve = Y.parse, He = K.lex;
//#endregion
export { X as Hooks, K as Lexer, Z as Marked, Y as Parser, q as Renderer, J as TextRenderer, G as Tokenizer, t as defaults, e as getDefaults, He as lexer, $ as marked, Fe as options, Be as parse, ze as parseInline, Ve as parser, Ie as setOptions, Le as use, Re as walkTokens };
