...
function z(r) {
  let e = [];
  return (t) => {
    let s = Math.max(0, Math.min(3, t - 1)), n = e[s];
...
function u(r, e = "") {
  let t = typeof r == "string" ? r : r.source, s = { replace: (n, l) => {
    ...
    return i = i.replace(x.caret, "$1"), t = t.replace(n, i), s;
...
function se(r, e) {
  let t = r.replace(x.findPipe, (l, i, a) => {
    let c = !1, o = i;
    for (; --o >= 0 && a[o] === "\\"; ) c = !c;
    return c ? "|" : " |";
...
function ie(r, e, t, s, n) {
  let l = e.href, i = e.title || null, a = r[1].replace(n.other.outputLinkReplace, "$1");
  s.state.inLink = !0;
  let c = { type: r[0].charAt(0) === "!" ? "image" : "link", raw: t, href: l, title: i, text: a, tokens: s.inlineTokens(a) };
...
var B = class {
  ...
  code(r) {
    ...
    if (e) {
      let t = this.options.pedantic ? e[0] : le(e[0]), s = t.replace(this.rules.other.codeRemoveIndent, "");
...
`), s = "", n = "", l = [];
      for (; t.length > 0; ) {
        let i = !1, a = [], c;
        for (c = 0; c < t.length; c++) if (this.rules.other.blockquoteStart.test(t[c])) a.push(t[c]), i = !0;
...
${p}` : p;
        ...
        if (this.lexer.state.top = !0, this.lexer.blockTokens(p, l, !0), this.lexer.state.top = h, t.length === 0) break;
        let g = l.at(-1);
...
` + t.join(`
`), y = this.blockquote(f);
          l[l.length - 1] = y, s = s.substring(0, s.length - d.raw.length) + y.raw, n = n.substring(0, n.length - d.text.length) + y.text;
          break;
        } else if (g?.type === "list") {
          let d = g, f = d.raw + `
` + t.join(`
`), y = this.list(f);
          l[l.length - 1] = y, s = s.substring(0, s.length - g.raw.length) + y.raw, n = n.substring(0, n.length - d.raw.length) + y.raw, t = f.substring(l.at(-1).raw.length).split(`
`);
          continue;
        }
      }
  ...
  list(r) {
    ...
    if (e) {
      ...
      let l = this.rules.other.listItemRegex(t), i = !1;
      for (; r; ) {
        let c = !1, o = "", p = "";
...
`, r = r.substring(g.length + 1), c = !0), !c) {
          let y = this.rules.other.nextBulletRegex(f), K = this.rules.other.hrRegex(f), V = this.rules.other.fencesBeginRegex(f), Y = this.rules.other.headingBeginRegex(f), xe = this.rules.other.htmlBeginRegex(f), be = this.rules.other.blockquoteBeginRegex(f);
          for (; r; ) {
            let D = r.split(`
`, 1)[0], P;
            if (g = D, this.options.pedantic ? (g = g.replace(this.rules.other.listReplaceNesting, "  "), P = g) : P = g.replace(this.rules.other.tabCharGlobal, "    "), V.test(g) || Y.test(g) || xe.test(g) || be.test(g) || y.test(g) || K.test(g)) break;
            if (P.search(this.rules.other.nonSpaceChar) >= f || !g.trim()) p += `
` + P.slice(f);
            else {
              if (d || h.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || V.test(h) || Y.test(h) || K.test(h)) break;
              p += `
` + g;
            }
            d = !g.trim(), o += D + `
`, r = r.substring(D.length + 1), h = P.slice(f);
          }
        }
        n.loose || (i ? n.loose = !0 : this.rules.other.doubleBlankLine.test(o) && (i = !0)), n.items.push({ type: "list_item", raw: o, task: !!this.options.gfm && this.rules.other.listIsTask.test(p), loose: !1, text: p, tokens: [] }), n.raw += o;
      }
      let a = n.items.at(-1);
  ...
  def(r) {
    ...
    if (e) {
      ...
      return { type: "def", tag: t, raw: R(e[0], `
`), href: s, title: n };
    }
  }
  table(r) {
    ...
    let t = se(e[1]), s = e[2].replace(this.rules.other.tableAlignChars, "").split("|"), n = e[3]?.trim() ? e[3].replace(this.rules.other.tableRowBlankLine, "").split(`
`) : [], l = { type: "table", raw: R(e[0], `
`), header: [], align: [], rows: [] };
    if (t.length === s.length) {
      ...
      for (let i of n) l.rows.push(se(i, l.header.length).map((a, c) => ({ text: a, tokens: this.lexer.inline(a), header: !1, align: l.align[c] })));
      return l;
    }
  }
  lheading(r) {
...
` ? e[1].slice(0, -1) : e[1];
  ...
  emStrong(r, e, t = "") {
    ...
    if (!(!s || !s[1] && !s[2] && !s[3] && !s[4] || s[4] && t.match(this.rules.other.unicodeAlphaNumeric)) && (!(s[1] || s[3]) || !t || this.rules.inline.punctuation.exec(t))) {
      let n = [...s[0]].length - 1, l, i, a = n, c = 0, o = s[0][0] === "*" ? this.rules.inline.emStrongRDelimAst : this.rules.inline.emStrongRDelimUnd;
      for (o.lastIndex = 0, e = e.slice(-1 * r.length + n); (s = o.exec(e)) !== null; ) {
        ...
        if (a -= i, a > 0) continue;
        i = Math.min(i, i + a + c);
        let p = [...s[0]][0].length, h = r.slice(0, n + s.index + p + i);
        if (Math.min(n, i) % 2) {
          let d = h.slice(1, -1);
  ...
  del(r, e, t = "") {
    ...
    if (s && (!s[1] || !t || this.rules.inline.punctuation.exec(t))) {
      let n = [...s[0]].length - 1, l, i, a = n, c = this.rules.inline.delRDelim;
      for (c.lastIndex = 0, e = e.slice(-1 * r.length + n); (s = c.exec(e)) !== null; ) {
        if (l = s[1] || s[2] || s[3] || s[4] || s[5] || s[6], !l || (i = [...l].length, i !== n)) continue;
        if (s[3] || s[4]) {
          a += i;
          continue;
        }
        if (a -= i, a > 0) continue;
        i = Math.min(i, i + a);
        let o = [...s[0]][0].length, p = r.slice(0, n + s.index + o + i), h = p.slice(n, -n);
  ...
  constructor(e) {
    ...
    let t = { other: x, block: E.normal, inline: _.normal };
...
`), this.blockTokens(e, this.tokens);
  ...
  blockTokens(e, t = [], s = !1) {
    ...
    for (; e; ) {
      ...
      let l;
      if (this.options.extensions?.block?.some((a) => (l = a.call({ lexer: this }, e, t)) ? (e = e.substring(l.raw.length), t.push(l), !0) : !1)) continue;
...
`) ? "" : `
`) + l.raw, a.text += `
` + l.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = a.text) : t.push(l), s = i.length !== e.length, e = e.substring(l.raw.length);
        continue;
      }
...
`;
  ...
  link({ href: r, title: e, tokens: t }) {
    let s = this.parser.parseInline(t), n = ne(r);
    if (n === null) return s;
    r = n;
    let l = '<a href="' + r + '"';
    return e && (l += ' title="' + m(e) + '"'), l += ">" + s + "</a>", l;
  }
  image({ href: r, title: e, text: t, tokens: s }) {
    ...
    r = n;
    let l = `<img src="${r}" alt="${m(t)}"`;
    return e && (l += ` title="${m(e)}"`), l += ">", l;
  }
  text(r) {
  ...
  parse(e) {
    ...
    for (let s = 0; s < e.length; s++) {
      ...
      if (this.options.extensions?.renderers?.[n.type]) {
        let i = n, a = this.options.extensions.renderers[i.type].call({ parser: this }, i);
    ...
    return r.forEach((t) => {
      ...
      if (s.async = this.defaults.async || s.async || !1, t.extensions && (t.extensions.forEach((n) => {
        ...
        if ("renderer" in n) {
          ...
          l ? e.renderers[n.name] = function(...i) {
      ...
      if (t.hooks) {
        ...
        for (let l in t.hooks) {
          ...
          let i = l, a = t.hooks[i], c = n[i];
          L.passThroughHooks.has(l) ? n[i] = (o) => {
  ...
  parseMarkdown(r) {
    return (e, t) => {
      ...
      if (n.hooks && (n.hooks.options = n, n.hooks.block = r), n.async) return (async () => {
        let i = n.hooks ? await n.hooks.preprocess(e) : e, a = await (n.hooks ? await n.hooks.provideLexer(r) : r ? b.lex : b.lexInline)(i, n), c = n.hooks ? await n.hooks.processAllTokens(a) : a;
      ...
      try {
        ...
        let i = (n.hooks ? n.hooks.provideLexer(r) : r ? b.lex : b.lexInline)(e, n);
        n.hooks && (i = n.hooks.processAllTokens(i)), n.walkTokens && this.walkTokens(i, n.walkTokens);
        let a = (n.hooks ? n.hooks.provideParser(r) : r ? w.parse : w.parseInline)(i, n);
...
k.options = k.setOptions = function(r) {
  return S.setOptions(r), k.defaults = S.defaults, ae(k.defaults), k;
};
k.getDefaults = O;
k.defaults = T;
k.use = function(...r) {
  return S.use(...r), k.defaults = S.defaults, ae(k.defaults), k;
};
...
k.Hooks = L;
k.parse = k;
var it = k.options, at = k.setOptions, ot = k.use, ct = k.walkTokens, ht = k.parseInline, pt = k, ut = w.parse, gt = b.lex;
export {
  L as Hooks,
  b as Lexer,
  lt as Marked,
