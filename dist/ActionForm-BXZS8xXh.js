...
import { At as t, Dt as n, It as r } from "./MultiDashboard-B8rxYV_S.js";
import { t as i } from "./useWatchAction-B2925xXY.js";
import { t as a } from "./useSubmitAction-vimZgsDR.js";
...
function j({ data: e, options: f, widgetId: _ }) {
	let { backendUrl: v, ctx: y, emit: b, requestRefresh: x, toast: S } = t(), C = s(() => p(e, f), [e, f]), w = f ?? {}, { submit: T, submitting: E, result: D } = a(_), O = C ? JSON.stringify(C.fields.map((e) => e.contextKey ? [e.contextKey, y[e.contextKey]] : null)) : "", k = s(() => C ? m(C, y) : {}, [C, O]), [A, j] = l(k), [P, F] = l({}), [I, L] = l(!1), [R, z] = l(!1), B = c(!1), [V, H] = l(null), [U, W] = l(null);
	if (o(() => {
		j(k), F({}), L(!1), H(null), W(null);
	}, [k]), !C) return /* @__PURE__ */ u(r, { children: "Action form requires fields" });
	let G = v === void 0 ? w.url ? "url" : null : "connect";
	if (!G) return /* @__PURE__ */ u(r, { children: "Action form requires backendUrl or options.url" });
	if (!C.actionId) return /* @__PURE__ */ u(r, { children: "Action form requires action_id" });
	let K = G === "connect" ? E : R, q = (e, t) => {
		j((n) => ({
			...n,
			[e]: t
		})), F((t) => {
		...
		let t = g(C, A);
		if (L(!1), H(null), W(null), G === "connect") {
			await T({
				actionId: C.actionId,
				params: t,
		...
		try {
			...
			b({
				...
				message: s,
				terminal: !0
			}), o ? (W(s), S(s, "error")) : (H(s), S(s, "ok"), w.refresh !== !1 && x(w.refresh_target ?? _ ?? "*"), Y());
		} catch (e) {
	...
	}, Z = D?.message ?? D?.status, Q = D && i(D.status) ? Z : null, $ = D && !i(D.status) ? Z : null;
		...
		children: [
			...
			/* @__PURE__ */ d("div", {
				...
				children: [I && /* @__PURE__ */ u("button", {
					...
					children: K ? "Working…" : I ? `Confirm ${C.submitLabel}` : C.submitLabel
				})]
			}),
			(G === "connect" ? $ : V) && /* @__PURE__ */ u("div", {
				className: "text-xs text-emerald-400",
				children: G === "connect" ? $ : V
			}),
			(G === "connect" ? Q : U) && /* @__PURE__ */ u("div", {
				className: "text-xs text-red-400",
				children: G === "connect" ? Q : U
			})
		]
	});
}
