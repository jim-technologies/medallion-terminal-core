import { it as e, ut as t } from "./MultiDashboard-B8rxYV_S.js";
...
function p(e, t) {
	if (e.type === "currency" || e.format?.startsWith("currency")) {
		let r = e.format?.startsWith("currency:") ? e.format.slice(9) : "USD";
		return n(t, r);
	}
	return e.type === "percent" || e.format === "percent" ? r(t) : e.format === "compact" ? i(t) : t.toLocaleString(void 0, { maximumFractionDigits: 4 });
}
...
function y({ field: t, value: n, onChange: r, disabled: i, compact: a, autoFocus: o, onCommit: u, onCancel: d }) {
	...
	if (t.type === "multi_select" || (t.type === "user" || t.type === "link") && t.choices.length > 0 && t.allowMultiple) {
		let e = Array.isArray(n) ? n.map(l) : [];
	...
	let f = t.type === "number" || t.type === "currency" || t.type === "percent", p = f ? "number" : t.type === "date" ? "date" : t.type === "datetime" ? "datetime-local" : t.type === "email" ? "email" : t.type === "phone" ? "tel" : t.type === "url" ? "url" : "text", y = t.type === "datetime" ? _(n) : g(n);
